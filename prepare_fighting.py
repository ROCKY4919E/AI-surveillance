...
OUTPUT_DIR   = 'data/fighting'
CRIME_FOLDER = 'Fighting'
...
def prepare():
    ...
    for split_name in ['train', 'test']:
        ...
        crime_clips  = collect_by_clip(split_src / CRIME_FOLDER)
        crime_total  = sum(len(v) for v in crime_clips.values())
        print(f"  {CRIME_FOLDER} clips : {len(crime_clips)}  ({crime_total} frames)")

        normal_clips_all = collect_by_clip(split_src / 'NormalVideos')
        normal_clips     = select_normal_by_frame_count(normal_clips_all, crime_total)
        normal_total     = sum(len(v) for v in normal_clips.values())
        print(f"  Normal clips       : {len(normal_clips)}  ({normal_total} frames)")

        if split_name == 'train':
            for clips, label in [(crime_clips, CRIME_LABEL), (normal_clips, 'normal')]:
