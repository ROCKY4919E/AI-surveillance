# prepare_roadaccidents.py
import os, shutil, random
from pathlib import Path
from collections import defaultdict

DATASET_ROOT = 'data/ucf_crime_subset'
OUTPUT_DIR   = 'data/roadaccidents'
CRIME_FOLDER = 'RoadAccidents'
CRIME_LABEL  = 'road_accident'
VAL_RATIO    = 0.15
RANDOM_SEED  = 42
...
def collect_by_clip(folder):
    ...
    for f in Path(folder).rglob('*.png'):
        clips[get_clip_id(f.name)].append(f)
    return clips
...
def split_clips(clips_dict, val_ratio):
    ...
    train_ids    = clip_ids[:split]
    val_ids      = clip_ids[split:]
    train_frames = [f for cid in train_ids for f in clips_dict[cid]]
    val_frames   = [f for cid in val_ids   for f in clips_dict[cid]]
...
def prepare():
    ...
    for split_name in ['train', 'test']:
        ...
        # RoadAccidents
        accident_clips = collect_by_clip(split_src / 'RoadAccidents')
        accident_total = sum(len(v) for v in accident_clips.values())
        print(f"  RoadAccidents clips : {len(accident_clips)}  ({accident_total} frames)")

        # Normal — match accident frame count
        normal_clips_all = collect_by_clip(split_src / 'NormalVideos')
        normal_clips     = select_normal_by_frame_count(normal_clips_all, accident_total)
        normal_total     = sum(len(v) for v in normal_clips.values())
        print(f"  Normal clips        : {len(normal_clips)}  ({normal_total} frames)")

        if split_name == 'train':
            for clips, label in [(accident_clips, 'road_accident'), (normal_clips, 'normal')]:
